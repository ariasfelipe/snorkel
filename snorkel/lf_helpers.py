...
def cross_sentence_get_text_splits(c, session):
    """
    Given a k-arity Candidate defined over k Spans, return the chunked parent
    context (e.g. Sentence) split around the k constituent Spans.
    """
...
def cross_sentence_get_tagged_text(c, session):
    """
    Returns the text of c's parent context with c's unary spans replaced with
    tags {{A}}, {{B}}, etc. A convenience method for writing LFs based on e.g.
    regexes.
    """
    return "".join(cross_sentence_get_text_splits(c, session))
...
def get_all_tokens(c, session, attrib='words', n_max=1, case_sensitive=False):
    ...
    sentences = get_sentences(c, session)
...
def cross_sentence_get_between_tokens(c, session, attrib='words', n_max=1, case_sensitive=False):
...
def cross_sentence_get_left_tokens(c, session, window=3, attrib='words', n_max=1, case_sensitive=False):
    """
    Return the tokens within a window to the _left_ of the Candidate.
    For higher-arity Candidates, defaults to the _first_ argument.
    :param window: The number of tokens to the left of the first argument to
...
def cross_sentence_get_right_tokens(c, session, window=3, attrib='words', n_max=1,
    case_sensitive=False):
    """
    Return the tokens within a window to the _right_ of the Candidate.
    For higher-arity Candidates, defaults to the _last_ argument.
...
def cross_sentence_rule_regex_search_tagged_text(candidate, pattern, sign, session):
    return sign if re.search(pattern, cross_sentence_get_tagged_text(candidate, session), flags=re.I) else 0
...
def cross_sentence_rule_regex_search_btw_AB(candidate, pattern, sign, session):
    return sign if re.search(r'{{A}}' + pattern + r'{{B}}', cross_sentence_get_tagged_text(candidate, session), flags=re.I) else 0
...
def cross_sentence_rule_regex_search_btw_BA(candidate, pattern, sign, session):
    return sign if re.search(r'{{B}}' + pattern + r'{{A}}', cross_sentence_get_tagged_text(candidate, session), flags=re.I) else 0
...
def cross_sentence_rule_regex_search_before_A(candidate, pattern, sign, session):
    return sign if re.search(pattern + r'{{A}}.*{{B}}', cross_sentence_get_tagged_text(candidate, session), flags=re.I) else 0
...
def cross_sentence_rule_regex_search_before_B(candidate, pattern, sign, session):
    return sign if re.search(pattern + r'{{B}}.*{{A}}', cross_sentence_get_tagged_text(candidate, session), flags=re.I) else 0
